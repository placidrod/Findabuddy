...
exports.getMessagesByRecipient = function(req, res) {
  console.log('Get Messages request param:', req.body);
...
var getUserAverageRating = function(req, res) {
...
//modify to be postBuddyRequest rating
exports.postUserRating = function(req, res) {
  User.findOne({_id: req.params._id})
    .exec(function(err, user) {
      if (user) {
        user.ratings.push(req.body.ratings);
        ...
        res.send('not updated. failed to find user');
      }
    });
};
